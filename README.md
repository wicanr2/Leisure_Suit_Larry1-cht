...
> ▶️ **預告片**：[EGA 中文版](https://youtu.be/5uPCoNFGz-c)　·　[VGA 中文版](https://youtu.be/epidQX9k3h8)（皆配原版遊戲音樂，中文標題 hero 片頭）
...
> **🎉 創舉：驗證一次通過，之後永久免答（EGA／VGA 雙版皆支援）。** 老遊戲每次開機都要重考一次成人問答，煩不勝煩。本中文化在引擎層動了手腳：**第一次通過年齡＋問答驗證後，之後開機自動幫你答過**（標題／警告片頭照常顯示，只是問答自動作答，你看著它一題題答對，然後直接進遊戲）。
>
> 核心理念一致——**不是背答案，是讓遊戲自己把正解告訴引擎**：
> - **EGA 版**：逆向出遊戲把「正確答案」暫存的內部變數 `v93`，引擎在驗證室（room 6）自動填入正解。
> - **VGA 版**：逆向出年齡問答（script 720）的題庫格式——每道成人常識題的文字，**第一個字元就是正解編號**（1–4），遊戲載入題目時複製進區域變數 `local[4]`。引擎讀出後，年齡選單以 TAB 導覽到成人選項、問答則直接按下對應字母鍵（a/b/c/d）作答；合成鍵優先於玩家輸入，即使你在旁邊亂按也不會誤選。
>
> 判定「已通過」靠：你已有本遊戲存檔，或首次通過時寫下的小旗標檔。全新玩家第一次仍照常手動作答（驗證機制本身不失效）。逆向與實作細節：EGA 見 [`docs/40-agi-track.md`](docs/40-agi-track.md) §7、VGA 見 [`docs/45-sci-age-verify.md`](docs/45-sci-age-verify.md)。

> **⌨️ 想直接跳過？按 `Ctrl+Alt+X`（EGA／VGA 雙版）。** 在驗證畫面按下，**一鍵直接跳進遊戲開場**（賴瑞站在老左酒吧外），不必答任何一題。原理是第一性原理最單純的解——**驗證只是一道「門」，與其逐題答對，不如直接走到門後的起始房間**：引擎打斷驗證室的控制流程、直接切換場景（EGA→room 11、VGA→room 100，用的是 ScummVM 內建的換場機制）。按下即視為通過並寫旗標，之後開機本來就自動略過。

> **遊戲中隨時按 `F8` 切換中／英文**：想對照原文台詞、或單純想看看老賴瑞當年的原汁英文冷笑話，按一下 F8 就把當前對話翻回英文，再按一下換回中文。EGA 版是**當前對話框原地即時切換**；VGA 版於**下一句對話生效**。

...
> **🎉 創舉：驗證一次通過，之後永久免答。** 老遊戲每次開機都要重考一次成人問答，煩不勝煩。本中文化在引擎層動了手腳：**第一次通過年齡＋問答驗證後，之後開機自動幫你答過**（標題／警告片頭照常顯示，只是問答自動作答，你看著它一題題答對）。原理是逆向出遊戲把「正確答案」暫存的內部變數（EGA 的 `v93`），引擎在驗證室自動填入正解——不是背答案，是讓遊戲自己告訴引擎答案。判定「已通過」靠：你已有本遊戲存檔，或首次通過時寫下的小旗標檔。全新玩家第一次仍照常手動作答（驗證機制本身不失效）。*(EGA 版已支援；VGA 版進行中。)*

...
**build 踩雷（硬）**：configure 時 `--disable-all-engines` 必須在 `--enable-engine=sci`（或 `agi`）**之前**；必加 `--disable-detection-full` + `--disable-mt32emu`。
...
5. **configure flag 順序**：`--disable-all-engines` 在 `--enable-engine=sci/agi` 前；必加 `--disable-detection-full` + `--disable-mt32emu`。

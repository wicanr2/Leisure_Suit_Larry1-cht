...
- **驗證法**：跑起來 log 出現 `Falling back to MT32`（Munt 先找 CM32L 才回退＝MT32 ROM 成功載入）且無 `cannot be used` 即成功。已用打包好的 EGA/VGA AppImage 實測通過（標題畫面正常、無阻擋框）；Windows zip 內含 ROM + .bat 帶 mt32 flag。
- **[雷] mingw 啟用 mt32emu 撞 `MT32EMU_VERSION_*` 未宣告**：`scummvm-win`（純目錄，非 git）用 `git diff --name-only HEAD` 從 scummvm-src 同步時只補 patch 改動檔，**漏掉 vendor 靜態檔 `audio/softsynth/mt32/config.h`**（Munt 版本標頭，當初 mt32 停用時 scummvm-win 沒這檔）→ 編譯失敗。修法：從 scummvm-src 補該檔（逐位元組相同）。啟用新子系統後、mingw 編譯前，`diff -rq` 比對 scummvm-src/scummvm-win 該子系統目錄（濾 build 產物）補缺檔。
